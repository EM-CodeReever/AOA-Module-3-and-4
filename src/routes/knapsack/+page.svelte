<script lang="ts">
    ...
    import { getBestPriorityVehicles } from "./Knapsack";
    ...
    function splitVehiclesEvenly(vehicles: Vehicle[], lanes: number) {
        ...
        for (let i = 0; i < lanes; i++) {
            let laneVehicles = vehicles.slice(
                i * vehiclesPerLane,
                (i + 1) * vehiclesPerLane,
            );
...
<p class="text-center">
    Group: <strong>Elliot Morrison, Matthew Cole, Garaine Baker</strong>
</p>
<p class="text-center">
    This program makes use of the knapsack algorithm to find the best route for
    vehicles travelling to and from kingston; considering their congestion
    impact, priority and the congestion limit of the highways.
</p>

<section class="flex p-5 space-x-5">
    <section class="flex space-y-2 w-full max-w-3xl flex-col">
        <!-- Mandela HighWay -->
        <p class="bg-indigo-600 rounded-lg p-2 text-white font-bold w-fit">
            Mandela Highway - Congestion limit: <input
                class="bg-gray-700 w-16 text-white p-2 rounded"
                bind:value={mandelaCapacity}
                type="number"
            />
        </p>
        <div class="bg-gray-800 h-80 max-h-fit w-full relative rounded">
            {#each splitVehiclesEvenly(vehicles, 4) as laneVehicles, i}
                <div
                    class="flex space-x-5 h-20 items-center px-5 w-full justify-end"
                >
        ...
        <!-- Portmore Toll -->
        <p class="bg-indigo-600 rounded-lg p-2 text-white font-bold w-fit">
            Portmore Toll - Congestion limit: <input
                class="bg-gray-700 w-16 text-white p-2 rounded"
                bind:value={portmoreCapacity}
                type="number"
            />
        </p>
    ...
    <section
        class="rounded mt-16 w-full space-y-3 w-full max-w-md bg-gray-800 p-3 h-fit flex flex-col"
    >
        ...
        <button
            on:click={() => {
                getBestVehicles(mandelaCapacity);
            }}
    ...
    <section>
        <div
            class="w-full max-w-lg mt-16 min-h-64 bg-gray-700 rounded p-2 text-gray-300"
        >
            {#if showOutput}
                <p>
                    {countVehicleTypes(vehicles)} were able to be placed on the Mandela
                    Highway {leftOverVehicles.length > 0
                        ? "to maximize value"
                        : ""} without exceeding its congestion limit of {mandelaCapacity}.
                    <br />
                </p>
                {#if leftOverVehicles.length > 0}
                    <p>
                        There were {leftOverVehicles.length} left-over vehicles placed
                        on the Portmore Toll to Kingston, which is more efficient
                        than adding them to the Mandela Hwy; consequently increasing
                        congestion.
                    </p>
                {:else}
                    <p>
                        All vehicles were placed on the Mandela Highway, without
                        exceeding its congestion limit 😁👌
                    </p>
                {/if}
            {:else}
